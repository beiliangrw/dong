...
Page({
      ...
      if (res.code === '0') {
        ...
        cartList.forEach(item => {
          ...
          if (item.fileIds) {
            ...
            imgSrc = 'http://192.168.1.102:8888/files/download/' + fileId;
          }
          item.url = imgSrc;
        })
        this.setData({
  ...
  handlePay(){
    ...
    if(this.data.addressInfo.id){
      let data = {addressId:this.data.addressInfo.id, userId: user.id, level: user.level, totalPrice: this.data.totalPrice, goodsList: this.data.cart};
      request({url: '/orderInfo/dis', method: 'POST', data:data}).then(res => {
        if (res.code === '0') {
          wx.showToast({
            title:"下单成功"
          })
