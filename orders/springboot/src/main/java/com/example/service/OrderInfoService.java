...
public class OrderInfoService {
    ...
    public PageInfo<OrderInfo> findPages(Long userId, Integer pageNum, Integer pageSize, HttpServletRequest request) {
        ...
        for (OrderInfo orderInfo : orderInfos) {
            packOrder(orderInfo);
            AddressInfo addressInfo = addressInfoDao.selectByPrimaryKey(orderInfo.getAddressId());
            orderInfo.setLinkAddress(addressInfo.getAddress());
            orderInfo.setLinkMan(addressInfo.getName());
            orderInfo.setLinkPhone(addressInfo.getPhone());
    ...
    public void add(OrderInfo info,  List<GoodsInfo> goodsList) {
    ...
    public void tuidan(Long id, Long userId) {
       changeStatus(id,"已退款");
    }

    public void cuidan(Long id, Long userId) {
        OrderInfo order = orderInfoDao.findById(id);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            Date date = sdf.parse(order.getCreateTime());
            Date afterDate = new Date(date.getTime() + 900000);
            order.setCreateTime(sdf.format(afterDate));
